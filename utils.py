...
def filter_genes_v2(adata, markers_dict, alarm_file):
    ...
    for k, (parent, p, n) in markers_dict.items():
        ...
        filtered_markers_dict[k] = (
            parent, positive_filtered_genes, negative_filtered_genes)
    if alarm_file:
        with open(alarm_file, 'w') as odata:
            for alarm in alarms:
                odata.write(alarm)
    if not filtered_markers_dict:
        raise Exception(
            "No valid marker gene is found in dataset! Please check whether your input species is right!")
...
def parse_markers_by_title(adata, marker_file, alarm_file=None):
    ...
    with open(marker_file, 'r') as indata:
        ...
        for line in indata:
            ...
            markers_dict[celltype] = list(
                set(markers_dict[celltype] + markers))
